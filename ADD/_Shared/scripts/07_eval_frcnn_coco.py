...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--yolo-split-name",
        type=str,
        default="test_internal",
        help="Folder name under images/ and labels/ when --split test_internal_yolo is used.",
    )
...
def build_coco_gt_from_yolo_split(split_root: Path, split_name: str) -> Path:
    images_dir = split_root / "images" / split_name
    labels_dir = split_root / "labels" / split_name
    out_gt = split_root / f"annotations_{split_name}_gt.json"

    images = []
    annotations = []
    ann_id = 1
    categories = [{"id": 1, "name": "car"}, {"id": 2, "name": "pedestrian"}, {"id": 3, "name": "cyclist"}]
...
def main() -> None:
    ...
    if args.split == "test_internal_yolo":
        gt_path = build_coco_gt_from_yolo_split(args.yolo_split_root, args.yolo_split_name)
        # build a dummy CocoDetection to reuse coco_eval
        coco_gt = COCO(str(gt_path))
        img_root = args.yolo_split_root / "images" / args.yolo_split_name
        to_tensor = transforms.ToTensor()
