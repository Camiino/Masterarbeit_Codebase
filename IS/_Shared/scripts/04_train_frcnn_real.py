#!/usr/bin/env python3
"""
Train Faster R-CNN (ResNet-50 FPN, ImageNet-pretrained) for IS real-only experiments.
...
Classes: hex, hex_socket, phillips, pozidriv, slotted, torx.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train Faster R-CNN for IS real-only YOLO splits")
    p.add_argument(
        "--yolo-split-root",
        type=Path,
    ...
    p.add_argument(
        "--split-dir",
        type=Path,
        default=IS_DATA_ROOT / "real_yolo_splits" / "splits",
...
def main() -> None:
    ...
    # Load COCO-pretrained backbone/head, then replace the predictor for 6 classes (+background).
