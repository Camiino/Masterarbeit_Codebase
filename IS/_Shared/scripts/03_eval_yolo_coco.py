#!/usr/bin/env python3
"""
COCO-style evaluation for IS YOLO test splits with a unified protocol:

- Overall COCO metrics: AP@[0.50:0.95], AP50 (plus AR breakdown)
- Per-class AP: hex, hex_socket, phillips, pozidriv, slotted, torx
...
- YOLO label ids: 0 hex, 1 hex_socket, 2 phillips, 3 pozidriv, 4 slotted, 5 torx.
...
def evaluate_dataset(
    ...
    class_summary = " | ".join(
        f"AP_{class_name}={out['per_class'][class_name]['AP_50_95']:.4f}"
        for class_name in CATEGORY_ID_TO_NAME.values()
    )
    logging.info(
        "[%s] SUMMARY: mAP=%.4f | AP50=%.4f | %s",
        name,
        out["all"]["AP_50_95"],
        out["all"]["AP_50"],
        class_summary,
